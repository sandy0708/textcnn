...
class TextCNN(nn.Module):
    def __init__(self, device, num_classes, window_size=[3,4,5]):
        super(TextCNN, self).__init__()
        num_filters = 100 
        
        embedding_matrix = self.load_embedding_vector()
    ...
    def forward(self, x):
        ...
        # result = F.softmax(logit, dim=1) # 두 라벨에 대해서 소프트맥스
        return logit #torch.argmax(result, dim=1)
